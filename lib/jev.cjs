...
function loadEnv() {
  ...
  for (const start of [process.cwd(), __dirname]) {
    ...
    if (!file) continue;
    try { process.loadEnvFile(file); } catch (_) { /* unreadable .env: fall through */ }
    if (process.env.OPENROUTER_API_KEY) return;
  }
}
...
class AnswerCache {
  ...
  static key(model, state, questions) {
...
function createClient(opts = {}) {
  ...
  async function decide(state, questions, callOpts = {}) {
    if (!apiKey) throw new JevError('OPENROUTER_API_KEY is not set (env or .env)');
    ...
    const key = useCache ? AnswerCache.key(useModel, state, questions) : null;
    if (useCache) {
  ...
  return { decide, map, summary, stats, model };
}
