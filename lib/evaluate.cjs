...
function score(results) {
  ...
  // When the lens has an escape option ("none"), how often is the best REAL option right? A model
  // that retreats to "none" on hard items (Kev-4B did on 83 of 240) looks far worse than it ranks.
  let withoutNone = null;
  if (rows.length && rows.every((r) => 'none' in (r.answer.probabilities || {}))) {
    const pickReal = (r) => Object.entries(r.answer.probabilities).filter(([k]) => k !== 'none').sort((a, b) => b[1] - a[1])[0][0];
    withoutNone = {
      accuracy: rows.filter((r) => pickReal(r) === r.item.label).length / rows.length,
      noneChosen: rows.filter((r) => r.answer.choice === 'none').length,
    };
  }
  return { n: rows.length, errors: results.length - rows.length, accuracy: acc, top2, bands, confusion, thresholds: thresholdsFor(rows, correct), withoutNone };
}

// The lowest confidence you can act on and still be right at least `target` of the time, and what
// share of items clears it. Thresholds tuned on one backend do not transfer to another (Kev's
// confidence runs lower than Jev's), so every eval measures them afresh. Needs 20+ answers.
function thresholdsFor(rows, correct, targets = [0.9, 0.95]) {
  const sorted = [...rows].sort((x, y) => y.answer.confidence - x.answer.confidence);
  return targets.map((target) => {
    ...
    sorted.forEach((r, i) => {
      ...
      // only cut between distinct confidences, and only once there are enough answers to mean something
      if (n >= 20 && right / n >= target && (!next || next.answer.confidence < r.answer.confidence)) {
        best = { target, confidence: r.answer.confidence, coverage: n / rows.length, accuracy: right / n };
      }
    });
    return best || { target, confidence: null, coverage: 0, accuracy: null };
  });
}
...
module.exports = { evaluate, labeledSet, stripLabels, sample, score, thresholdsFor };
