...
async function route(jev, corpus, config, { since, pack, concurrency, today = new Date().toLocaleDateString('en-CA') } = {}) {
  ...
  const t0 = Date.now();
  const results = await siftMany(jev, items, lensMap, { pack, concurrency });
